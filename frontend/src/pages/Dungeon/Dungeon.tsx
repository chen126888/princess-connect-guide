...
const Dungeon: React.FC = () => {
  ...
  const renderTextWithIcons = (text: string) => {
    ...
    for (const key in iconMap) {
      ...
      const imgTags = iconNames.map(iconName => `<img src="http://localhost:3000/images/icons/${iconName}.png" alt="${iconName}" class="inline-block w-5 h-5 mx-1 align-middle" />`).join('');
