...
const ClanBattleFuture: React.FC = () => {
  ...
  const ensureFutureSightClanBattles = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/clan-battles/ensure-future-sight`, {
        method: 'POST',
        headers: {
  ...
  const loadClanBattles = async () => {
    try {
      ...
      // 然後載入所有戰隊戰資料
      const response = await fetch(`${API_BASE_URL}/clan-battles`);
      const result = await response.json();
  ...
  const handleAddTeams = async (data: any) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/clan-battles/batch-teams`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">戰隊戰未來視</h2>
          {isAdmin && (
            <button
...
export default ClanBattleFuture;